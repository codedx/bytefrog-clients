...
object MethodIdMapJson {
	implicit val jsonFormats = Serialization.formats(NoTypeHints)
	private case class MethodId(id: Int, sig: String)
	private case class MethodIdList(methods: List[MethodId])

	def serialize(segments: collection.Map[Int, String]): String = {
		val obj = MethodIdList(segments.map(MethodId.tupled).toList)
		write(obj)
	}

	def deserialize(input: String): Map[Int, String] = {
		try {
			val obj = read[MethodIdList](input)
			obj.methods.map(m => (m.id, m.sig)).toMap
		} catch {
