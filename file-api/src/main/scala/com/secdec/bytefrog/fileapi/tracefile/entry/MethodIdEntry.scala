...
import com.secdec.bytefrog.fileapi.data.MethodIdMapJson
...
object MethodIdEntry extends TraceFileEntry[Map[Int, String]] {

	val path = "method-ids.json"
	...
	private class Reader extends TraceFileEntryReader[Map[Int, String]] {
		def read(content: InputStream)(callback: Map[Int, String] => Unit): Unit = {
			try {
				...
				val parsed = MethodIdMapJson.deserialize(s)
				callback(parsed)
			} catch {
	...
	private class Writer(token: ZipFileBuilderToken) extends TraceFileEntryWriter[Map[Int, String]] {
		...
		def finish(): Unit = {
			val s = MethodIdMapJson.serialize(methodIds)
