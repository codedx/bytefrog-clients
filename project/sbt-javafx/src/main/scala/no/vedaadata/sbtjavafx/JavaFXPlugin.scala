...
object JavaFXPlugin extends Plugin {
  ...
  val packageJavaFxTask = (jfx, name, internalDependencyClasspath in Runtime, fullClasspath in Runtime, baseDirectory, crossTarget) map {
    (jfx, name, attributedClassDirs, fullClasspath, baseDirectory, crossTarget) =>
      ...
      // Include the internal dependencies
      val classDirs = attributedClassDirs map (_.data) filter {_.isDirectory}
        ...
        <project name={ name } default="default" basedir="." xmlns:fx="javafx:com.sun.javafx.tools.ant">
          <target name="default">
            ...
            <fx:jar destfile={ jarFile.getAbsolutePath }>
              ...
              {
                classDirs map { classDir => <fx:fileset dir={ classDir.getAbsolutePath } /> }
              }
              <fx:resources>
  ...
  //	Base settings that must be manually loaded

  val jfxBaseSettings = Seq(
    JFX.devKit := None,
