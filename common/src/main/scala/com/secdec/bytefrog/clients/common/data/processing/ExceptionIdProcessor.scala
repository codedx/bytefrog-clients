...
object ExceptionIdProcessor {
	def apply(output: TraceFileBuilder) = new ExceptionIdProcessor(output)
}
...
/** The MethodIdProcessor maintains a mapping of method IDs to their full signatures.
  ...
  * @author robertf
  */
class ExceptionIdProcessor(output: TraceFileBuilder) extends DataProcessor {
	private val exceptions = mutable.Map[Int, String]()

	def processMessage(message: DataMessageContent) = message match {
		case DataMessageContent.MapException(exc, id) =>
			exceptions.put(id, exc)
	...
	def finishProcessing() {
		val writer = TraceFile.exceptionIdEntry.writer(output)
